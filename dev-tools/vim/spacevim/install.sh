...
fetch_repo () {
  if [[ -d "$HOME/.SpaceVim" ]]; then
    ...
    # git clone https://github.com/SpaceVim/SpaceVim.git "$HOME/.SpaceVim"
     git clone https://gitee.com/proby143/SpaceVim.git "$HOME/.SpaceVim"
...
install_package_manager () {
  if [[ ! -d "$HOME/.cache/vimfiles/repos/github.com/Shougo/dein.vim" ]]; then
    ...
    git clone https://gitee.com/proby143/dein.vim.git $HOME/.cache/vimfiles/repos/github.com/Shougo/dein.vim
...
check_requirements () {
  ...
  info "正在检测终端真色支持..."
  # sh -c "$(curl -fsSL https://raw.staticdn.net/JohnMorales/dotfiles/master/colors/24-bit-color.sh)"
   sh -c "$(curl -fsSL https://raw.staticdn.net/JohnMorales/dotfiles/master/colors/24-bit-color.sh)"
...
# download_font {{{
download_font () {
  url="https://raw.staticdn.net/wsdjeg/DotFiles/7a75a186c6db9ad6f02cafba8d4c7bc78f47304c/local/share/fonts/${1// /%20}"
